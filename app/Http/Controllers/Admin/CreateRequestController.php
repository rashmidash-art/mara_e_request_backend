...
class CreateRequestController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($request, $req) {
            ...
            // ❌ Case: Checkbox only but no existing records
            if (
                $request->is_delivery_completed == 1 &&
                ! $hasAnyField &&
                ! $existingDeliveryExists
            ) {
                throw new \Exception('At least one delivery detail is required before marking complete');
            }

            // ❌ Case: Partial data → force all fields
            if ($hasAnyField) {
                $request->validate([
            ...
            if ($request->hasFile('delivery_documents')) {
                ...
                $fileName = 'delivery_'.$req->request_id.'_'.time().'_'.$file->getClientOriginalName();
                $file->storeAs('request_documents', $fileName, 'public');
            }

            // ✅ Save
            if ($request->is_delivery_completed == 1 || $hasAnyField) {
                DeliveryOrerDetails::create([
                    'request_id' => $req->request_id,
                    'is_delivery_completed' => $request->is_delivery_completed == 1 ? 1 : 0,
                    'delivery_number' => $request->delivery_number ?? null,
                    'delivery_date' => $request->delivery_date ?? null,
                    'delivery_quantity' => $request->delivery_quantity ?? null,
                    'delivery_documents' => $fileName,
                    'status' => 'completed',
                ]);
    ...
    public function requestDetailsAll(Request $request)
    {
        try {
            ...
            $data = $requests->map(function ($req) {
                ...
                /*
                |--------------------------------------------------------------------------
                |  Submitted (only if not draft or withdraw)
                |--------------------------------------------------------------------------
                */
                ...
                */
                ...
                /*
                |--------------------------------------------------------------------------
                |  Withdraw - REMOVED THE EARLY RETURN
                |--------------------------------------------------------------------------
                */
                if ($req->status === ModelsRequest::WITHDRAW) {
                    ...
                    /*
                    |--------------------------------------------------------------------------
                    |  Workflow Steps (only if not withdrawn)
                    |--------------------------------------------------------------------------
                    */
